...
# Subnet per Private Endpoint del Log Analytics storage
module "subnet_log_analytics" {
  source        = "Azure/avm-res-network-virtualnetwork/azurerm//modules/subnet"
  version       = "0.16.0"
  name          = "subnet-loganalytics-pe"
  parent_id     = module.vnet_log_analytics.resource.id
  address_prefixes = ["10.2.1.0/24"]
}

resource "azurerm_subnet_network_security_group_association" "subnet_loganalytics_nsg" {
